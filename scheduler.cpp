...
void Scheduler::be_a_job(int job_socket_id, std::string queue_name, long client_job_id, std::string data) {
    this->create_queue_if_not_exists(queue_name);
    struct Queue *queue = this->queues.find(queue_name)->second;
...
void Scheduler::remove_client(int socket_id) {
    ...
    while (qit != queues.end()) {
        ...
        std::list<int>::iterator ww_it = std::find(queue->waiting_workers.begin(), queue->waiting_workers.end(), socket_id);
                ...
                if (link->job_socket_id == socket_id || link->worker_socket_id == socket_id) {
                    ...
                    if (link->worker_socket_id == -1) {
                        ...
                        if (link->worker_socket_id == socket_id) {
                            ...
                            Socket::emit(link->job_socket_id, Socket::DONE_IT, queue_name, link->c_id, job_data);
                        } else {
        ...
        printf("- wj: %ld, ww: %ld, jobs: %ld, links: %ld\n", queue->waiting_jobs.size(), queue->waiting_workers.size(), queue->jobs.size(), queue->links.size());
        qit++;
    }
}
